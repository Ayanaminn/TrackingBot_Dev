...
class MainWindow(QtWidgets.QMainWindow, mainGUI.Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.videoThread.timeSignal.signal.connect(self.updateLabel)
        ...
        self.loadImgButton.clicked.connect(self.workerThread)
       ...
       try:
           ...
           # if no file selected
            if self.video_file[0] == '':
                ...
                self.videoThread.file = self.video_file
    ...
    def playVideo(self):
        # self.playCapture.open(self.video_file[0])
        self.videoThread.playCapture.open(self.video_file[0])
        self.videoThread.start()
        self.playButton.setIcon(self.style().standardIcon(QStyle.SP_MediaPause))
        self.status = MainWindow.STATUS_PLAYING
    ...
    def stopVideo(self):
        ...
        print(f'thread stop status is {is_stopped}')
        if is_stopped:
            # self.playCapture.release()
            self.videoThread.playCapture.release()
            # print('vid release')
...
class VideoThread(QThread):
    ...
    def run(self):
        ...
        while True:
            ...
            # print(f'vid thread file {self.file}')
            ...
            # while True:
                ...
                # frame_display = QPixmap.fromImage(frame_scaled)
                self.timeSignal.signal.emit(frame_scaled)
                # time.sleep(1/25)
                # print('ret')
